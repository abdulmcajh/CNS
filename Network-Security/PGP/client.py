...
def PGP_unpack(m,server_pubkey):
	...
	if server_pubkey.verify(tag,rtag):
...
def receiver(s,server_pubkey):
	while True:
		text=s.recv(4096)
		print "--Received PGP encoded_msg \n",text
		plain =PGP_unpack(text,server_pubkey)
...
def client(port):
	...
	p1.start()
	p2=threading.Thread(name='receiver',target=receiver,args=(s,server_pubkey,))
	p2.start()
